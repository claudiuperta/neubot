...
'''
 The main() of `neubot skype` subcommand.
'''
...
from neubot.skype_negotiate import SkypeNegotiate
...
def main(args):
    ...
    if runner:
        result = runner_clnt.runner_client(CONFIG['agent.api.address'],
          CONFIG['agent.api.port'], CONFIG['verbose'], 'skype')
        if result:
            sys.exit(0)
    ...
    if not fakeprivacy and not privacy.allowed_to_run():
        privacy.complain()
        logging.info('skype: otherwise use -y option to temporarily provide '
                     'privacy permissions')
        sys.exit(1)
