...
'''
 Client-side `negotiate` and `collect` phases of the skype test.
 Invokes code in skype_clnt.py for the `test` phase.
'''

# Adapted from neubot/bittorrent/client.py

#
# This file contains the negotiator client of the Skype test, i.e. the code that
...
from neubot.skype_clnt import SkypeClient
from neubot.state import STATE

from neubot import http_utils
from neubot import skype_analyze
...
class SkypeNegotiate(HttpClient):

    ''' Skype negotiate client '''
    ...
    def _start_test(opaque):
        ...
        logging.info('[ Skype ] ...starting test')
        ...
        logging.info('[ Skype ] ...starting SkypeClient()')
        client = SkypeClient()
    ...
    def _handle_test_success(self, stream, state):
        ...
        logging.debug('skype_negotiate: test complete... success')
        result = {
                  'al_capacity': skype_analyze.compute_bottleneck_capacity(
                     state['rcvr_data'], state['mss']),
                  'al_mss': state['mss'],
                  'al_rexmits': skype_analyze.select_likely_rexmits(
    ...
    def _handle_test_failure(stream, error):
        ...
        logging.warning('skype_negotiate: test failed: %s', str(error))
        stream.close()

    def _start_collect(self, stream, result):
        ''' Start the COLLECT phase '''
        STATE.update('collect')
        logging.debug('skype_negotiate: collect in progress...')
        context = stream.opaque
        extra = context.extra
        extra['local_result'] = result
        body = six.b(json.dumps(result))
        host_header = utils_net.format_epnt((extra['address'], extra['port']))
        self.append_request(stream, 'POST', '/collect/skype', 'HTTP/1.1')
        self.append_header(stream, 'Host', host_header)
        self.append_header(stream, 'User-Agent', utils_version.HTTP_HEADER)
        self.append_header(stream, 'Content-Type', 'application/json')
        self.append_header(stream, 'Content-Length', str(len(body)))
