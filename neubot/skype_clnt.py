...
'''
 Client-side `test` phase of the skype test.  Typically invoked by code that
 lives in skype_negotiate.py.
'''
...
from neubot.skype_defs import FAKEAUTH
from neubot.skype_defs import PING
from neubot.skype_defs import PINGBACK
from neubot.skype_defs import PINGBACK_CODE
from neubot.skype_defs import RAWTEST
from neubot.state import STATE
from neubot.udp_stream import UDPStream
...
class SkypeClient(Handler):

    ''' Skype test client '''

    def handle_connect(self, connector, sock, rtt, sslconfig, state):
        logging.info('skype_clnt: connection established with %s', connector)
        logging.info('skype_clnt: connect_time: %s', utils.time_formatter(rtt))
        state['connect_time'] = rtt
        # UDPStream()
        #Stream(sock, self._connection_ready, self._connection_lost,
        #  sslconfig, '', ClientContext(state))
        STATE.update('test', 'skype')
    ...
    def _connection_ready(self, stream):
        ...
        logging.info('skype_clnt: sending auth to server... in progress')
        context = stream.opaque
        auth = context.state.get('authorization')
        if not auth:
            logging.warning('skype_clnt: no auth, sending fake auth')
            auth = FAKEAUTH
        logging.debug('> AUTH')
    ...
    def _auth_sent(self, stream):
        ...
        logging.info('skype_clnt: sending auth to server... complete')
        logging.info('skype_clnt: receiving auth from server... in progress')
    ...
    def _waiting_auth(self, stream, data):
        ...
        #if tmp != FAKEAUTH:
        #    logging.warning('skype_clnt: nonfake auth from server')
        #logging.info('skype_clnt: receiving auth from server... complete')
        #logging.info('skype_clnt: estimating ALRTT... in progress')
    ...
    def _skypetest_sent(self, stream):
    ...
    def _periodic_error(stream, err):
        ...
        logging.warning('skype_clnt: _periodic_internal() failed: %s', err)
        stream.close()
    ...
    def _connection_lost_internal(stream):
        ...
        if not stream.opaque:
            raise RuntimeError('skype_clnt: no stream opaque')
        context = stream.opaque
        tmp = context.state.get('complete')
        if not tmp:
            raise RuntimeError('skype_clnt: connection unexpectedly closed')
    ...
    def _connection_lost_error(stream, err):
        ...
        logging.warning('skype_clnt: _connection_lost_internal() failed: %s', err)
        context = stream.opaque
        if context:
